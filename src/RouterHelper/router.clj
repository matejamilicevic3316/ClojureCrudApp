...
(GET "/search" {params :query-params} (session/logged-in (fn [_] (renderHtml "search.html" (product-handler/search-products (get params "page") (get params "producttypeid") (get params "keyword")))) "/login"))
...
(POST "/editproduct" [] (session/is-admin (fn [_] (product-handler/handle-product {:productid (:productid (:params _)) :description (:description (:params _)) :name (:name (:params _)) :producttypeid (:producttypeid (:params _)) :image (:file (:params _)) :price (:price (:params _))})) "/404"))
